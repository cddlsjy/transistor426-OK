...
abstract class BasePlayerFragment: Fragment(),
    ...
    override fun onResume() {
        ...
        remoteControlEnabled = PreferencesHelper.loadRemoteAutoPlay()
        if (remoteControlEnabled && collection.stations.isNotEmpty()) {
            collectionAdapter.setSelectedPosition(playerState.stationPosition)
        }
    }
        ...
        if (remoteControlEnabled && collection.stations.isNotEmpty()) {
            when (keyCode) {
                ...
                KeyEvent.KEYCODE_MEDIA_NEXT -> {
                    val nextPosition = (playerState.stationPosition + 1) % collection.stations.size
                    playerState.stationPosition = nextPosition
                    collectionAdapter.setSelectedPosition(nextPosition)
                    controller?.play(requireContext(), nextPosition)
                    return true
                }
                KeyEvent.KEYCODE_MEDIA_PREVIOUS -> {
                    val prevPosition = if (playerState.stationPosition > 0) {
                        playerState.stationPosition - 1
                    } else {
                        collection.stations.size - 1
                    }
                    playerState.stationPosition = prevPosition
                    collectionAdapter.setSelectedPosition(prevPosition)
                    controller?.play(requireContext(), prevPosition)
                    return true
                }
                KeyEvent.KEYCODE_DPAD_UP -> {
                    val currentPos = collectionAdapter.getSelectedPosition()
                    if (currentPos > 0) {
                        collectionAdapter.setSelectedPosition(currentPos - 1)
                        layout.layoutManager.scrollToPosition(currentPos - 1)
                    }
                    return true
                }
                ...
                KeyEvent.KEYCODE_DPAD_CENTER, KeyEvent.KEYCODE_ENTER -> {
                    val selectedPos = collectionAdapter.getSelectedPosition()
                    if (selectedPos >= 0) {
                        onPlayButtonTapped(selectedPos)
                    }
                    return true
                }
            }
