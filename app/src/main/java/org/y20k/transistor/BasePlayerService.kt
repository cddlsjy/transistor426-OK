...
//    }
    ...
    private fun updateMetadata(metadata: String = String()) {
        ...
        // remove HTML encoding and handle various character encodings
        val metadataString: String = try {
            ...
            // Fallback: try to handle encoding issues directly
            metadataStringEncoded
        }
        // Additional encoding fix: replace common problematic characters
        val cleanedMetadata = metadataString
            .replace("&amp;", "&")
            .replace("&lt;", "<")
            .replace("&gt;", ">")
            .replace("&quot;", "\"")
        ...
        // remove duplicates
        if (metadataHistory.contains(cleanedMetadata)) {
            metadataHistory.removeIf { it == cleanedMetadata }
        }
        // append metadata to metadata history
        metadataHistory.add(cleanedMetadata)
        // trim metadata list
